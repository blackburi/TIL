...
* 부분집합
    ...
    * 각 원소가 부분집합에 포함되어 있는지 loop를 이용하여 확인하고 부분집합을 생성하는 방법
        ...
        ```
* bit의 이해
    * 기본 이해 : 리스트 A가 존재할 때, 리스트의 인덱스를 2의 거듭제곱으로 역순으로 나타낸다. 이때 인덱스의 합을 기준으로 부분집합을 나타내는 방식으로 이해를 하면 된다. 2의 거듭제곱의 합으로 숫자를 나타내는 방법은 유일하기 때문에 숫자 한개당 부분집합을 표현하는 방법이 유일하다.
